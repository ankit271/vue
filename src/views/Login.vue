...
const handleGoogleLogin = async () => {
  try {
    ...
    try {
      ...
      console.log('Successfully logged in with popup:', user.email)
      router.push('/home')
    } catch (popupError) {
    ...
    switch (error.code) {
      case 'auth/unauthorized-domain':
        console.error('This domain is not authorized in Firebase')
        break
      case 'auth/operation-not-allowed':
        console.error('Google sign-in is not enabled in Firebase')
        break
      default:
        console.error('Other authentication error:', error.message)
    }
  } finally {
...
onMounted(async () => {
  try {
    // Check for redirect result as soon as component mounts
    const result = await getRedirectResult(auth)
    if (result) {
      const user = result.user
      console.log('Successfully logged in with redirect:', user.email)
      router.push('/home')
    }
  } catch (error) {
